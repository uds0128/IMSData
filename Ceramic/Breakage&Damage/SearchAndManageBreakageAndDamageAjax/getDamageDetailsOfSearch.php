    ...
    if($getStocks){
        if(mysqli_num_rows($getStocks) > 0){
            ...
            while($row = $getStocks->fetch_assoc()){
                ...
                $obj = array('billingqty' => $billing_qty, 'otherqty' => $other_qty, 'baseprice' => $base_price, 'dateadded' => $dateadded, 'stockid' => $stock_id, 'sysid' => $sysid, 'dbillingqty' => $dbillingqty, 'dotherqty' => $dothergqty, 'dcreateddate' => $dcreateddate);
                $dataToBeSent[] =$obj;
            }
