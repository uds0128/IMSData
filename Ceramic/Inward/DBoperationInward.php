...
if (isset($_GET['log1'])) {
    ...
    $sqlinsert           = "INSERT INTO TblInwardBillMst (InwardDate, VendorId, TotalGST, Transport_extracost, TotalAmount, AmountPaid, AmountPending, UploadBill, PaymentMode, TotalDiscount, Notes, StockMstSysId, ModifiedDate,  StockType) VALUES ('$InwardDate', '$VendorId', '$TotalGST', '$Transport_extracost', '$TotalAmount', '$AmountPaid', '$AmountPending', 0, '$PaymentMode', '0', '$Notes', '0', '0', '$StockType')";
    ...
    for ($x = 1; $x <= $count; $x++) {
        if (isset($_POST['type_' . $x])) {
            ...
            $sql1 .= "SELECT * FROM productmst where ProductSubCategoryID='$typeid' and ProductTypeColor='$color' and BrandId='$brandid' and PackingUnit='$unit' and GradeId='$gradeid' and Code='$code' and QtyPerUnit = '{$size}'";
    ...
    while ($row = mysqli_fetch_assoc($res1)) {
        ...
        for ($x = 1; $x <= $count; $x++) {
            if (isset($_POST['type_' . $x])) {
                ...
                $disc  = $_POST['disc_' . $x];
                //echo $sql;
                $checksys = "select * from systable where ProductId='$pid' and BasePrice='$price' and BatchNo='$batch'";
                $check1 = "select COUNT(*) as total, SysId from systable where ProductId='$pid' and BasePrice='$price' and BatchNo='$batch'";
                ...
                {   
                    ...
                    $insertsys .= "INSERT INTO systable (ProductId, BasePrice, BatchNo) VALUES ($pid, $price, $batch);"; 
